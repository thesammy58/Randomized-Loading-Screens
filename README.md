...
In order to make my loading screens still scale with a user's chosen Tiny UI Fix setting, a workaround had to be put in place. The ASI will check to see if a Tiny UI Fix package exists in the Mods/TinyUIFix directory. If one does, the ASI will harvest the randomly chosen loading screen's matching native LAYO (native meaning that SPECIFIC expansion/stuff pack's layout resource ID) from Tiny UI Fix, and inject that scaled LAYO in its entirety back in to our chosen loading screen by generating a second package file containing said scaled LAYOs with a higher loading priority. Since my loading screens each have 21 identical LAYO entries (so that the loading screen looks the same regardless of the user's latest installed pack), each package file has been stamped with a unique marker resource, identifying to the ASI exactly which specific pack's LAYO should be harvested from Tiny UI Fix and injected back into all 21 of our LAYOs.
...
1. Mods directory override inside the code (left blank for public release)
2. An optional .ini file specifying folder language (downloaded by the user if detection methods fail)
3. The game's registry entry
4. The user's operating system locale
5. Simply assume the folder is titled "The Sims 3" when all else fails
